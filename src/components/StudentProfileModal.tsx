...
export const StudentProfileModal: React.FC<StudentProfileModalProps> = ({
  ...
  const handleSaveBasicInfo = async () => {
    if (!student || !currentUser) return;
    if (!isValidDirection(editFaculty)) {
      onNotify('error', "Iltimos, 18 ta rasmiy ta'lim yo'nalishidan birini tanlang.");
      return;
    }
    const canonicalDir = canonicalizeDirection(editFaculty);
    setIsSaving(true);
    try {
          ...
          {!student ? (
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-5 pr-8">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm text-blue-100/90 mt-1 break-words">
                  {canonicalizeDirection(student.facultyOrField) || student.facultyOrField || "Ta'lim yo'nalishi ko'rsatilmagan"} • Guruh: <span className="font-mono font-bold text-white">{student.group}</span>
                </p>
                    ...
                    {isEditingBasic ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                        ...
                        <div className="space-y-1 sm:col-span-2">
                          <span className="text-slate-400">Ta’lim yo‘nalishi (18 rasmiy yo‘nalish):</span>
                          <p className="font-semibold text-slate-900 text-sm break-words bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                            {canonicalizeDirection(student.facultyOrField) || student.facultyOrField || "Ko'rsatilmagan"}
                          </p>
                        </div>
                        <div className="space-y-1">
