...
export const ProfilePhotoUploader: React.FC<ProfilePhotoUploaderProps> = ({
  ...
  const [localPhoto, setLocalPhoto] = useState<string | undefined>(currentPhotoUrl);
  ...
  const activePhoto = localPhoto || currentPhotoUrl;
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const previousPhotoUrl = activePhoto;
    ...
    try {
      ...
      // Immediately display image locally for instant, glitch-free UI update
  ...
  const handleDeletePhoto = async () => {
    const photoToDelete = activePhoto;
    if (!photoToDelete) return;
    try {
      setIsDeleting(true);
      setUploadError(null);
      await deleteProfilePhotoFromStorage(photoToDelete, userId);
      setLocalPhoto(undefined);
      setImgLoadFailed(false);
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
      ...
      <div className="relative group shrink-0">
        ...
        >
          {activePhoto && !imgLoadFailed ? (
            <img
              src={activePhoto}
              alt={userName}
              className="w-full h-full object-cover"
              onError={() => {
                console.warn('Profile image failed to render, falling back to initial');
              ...
              >
                {isUploading ? (
                  ...
                  <>
                    ...
                    <span>{activePhoto ? 'Rasmni almashtirish' : 'Rasm yuklash'}</span>
                  </>
                )}
              </button>
